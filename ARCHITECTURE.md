...
- **Embeddings** — originally local, via `langchain-huggingface`'s `HuggingFaceEmbeddings` running `sentence-transformers/all-MiniLM-L6-v2`, chosen specifically because indexing thousands of chunks via an API would burn through a free-tier request quota fast. **Replaced with Gemini's embeddings API** (`langchain-google-genai`'s `GoogleGenerativeAIEmbeddings`, model `gemini-embedding-001`) after a real Render free-tier (512MB) OOM — measured in a locally-capped-at-512MB Docker container that a single process just *importing* the RAG module chain (torch + transformers + sentence-transformers + chromadb) already sat at ~98% of the limit before serving a request, and that `torch` was resolving to the default CUDA-enabled Linux wheel (pulling in the full nvidia cublas/cudnn/cufft/cusolver stack) despite the app only ever doing CPU inference. No process restructuring fixed it - the dependency stack itself didn't fit. The trade-off predicted at the time (API embedding quota) turned out to be exactly right: Gemini's free tier caps `embed_content` at 100 requests/minute *and* 1000/day, so `scripts/build_index.py` batches and paces its calls instead of one big `Chroma.from_documents()` call, with backoff for the per-minute cap and a fast-fail for the per-day one (confirmed live - the day's testing exhausted the daily cap, which no amount of backoff clears).
...
- **Reranking** (`rag/retrieval.py`, `build_reranked_retriever`) — originally rescored the merged hybrid candidates with a local cross-encoder (`cross-encoder/ms-marco-MiniLM-L-6-v2` via `HuggingFaceCrossEncoder` + `CrossEncoderReranker`) before trimming to `top_n`, added after testing showed the raw hybrid merge included some weak keyword-only matches. **Dropped** in the same memory-driven change above — `build_reranked_retriever` now just trims the RRF-merged hybrid results to `top_n` directly, no model-based rescoring. Not yet re-evaluated against the golden set (see "Known limitations").
- **Tool-level error handling**: `/chat` (`api/main.py`) has no top-level try/except around the agent graph invocation, so any uncaught exception from a tool - including an embedding-API failure inside `search_fastapi_docs` - surfaces as a bare 500 for the whole request instead of a graceful in-chat message. Found live after the embeddings switch above (a question needing doc search 500'd once the daily quota was exhausted, while a question the LLM could answer without a tool call worked fine). Fixed at the tool level: `search_fastapi_docs` now catches retrieval exceptions and returns a plain-text fallback instead of raising.
...
`uv`-managed project, Python 3.13. Every dependency added via `uv add` so versions are resolved live against PyPI and locked in `uv.lock` — no hand-typed version guesses. Current core deps: `langchain==1.3.13`, `langgraph==1.2.9`, `langsmith==0.10.3`, `chromadb==1.5.9`, `langchain-chroma==1.1.0`, `langchain-groq`, `langchain-google-genai`, `langchain-community`, `rank-bm25`. `torch`/`transformers`/`sentence-transformers`/`langchain-huggingface` were removed entirely when embeddings moved to Gemini's API (see "Retrieval pipeline" above) - that alone dropped `uv.lock` from 155 to 119 packages and the Docker image from 1.52GB to 686MB.
...
- **Gemini embeddings free-tier quota (from the Render OOM fix, below)**: 100 requests/minute and 1000/day on `embed_content`. `build_index.py`'s own batching/pacing stays under the per-minute cap, but the per-day cap is shared with every query-time `embed_query` call too and doesn't reset for hours once hit - `search_fastapi_docs` degrades to a plain-text fallback rather than crashing when this happens, but doc search is genuinely unavailable until the quota resets.
